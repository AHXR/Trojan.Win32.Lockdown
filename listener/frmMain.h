...
#pragma region Windows Form Designer generated code
		...
		void InitializeComponent(void)
		{
			...
			// btnTest
			// 
			this->btnTest->Location = System::Drawing::Point(573, 36);
			...
			this->btnTest->Click += gcnew System::EventHandler(this, &frmMain::btnTest_Click);
			// 
			// btnUpdate
			// 
...
private: System::Void btnTest_Click(System::Object^  sender, System::EventArgs^  e) {
	sendNewMessage("ANDRE-PC (Windows 7) - Locked out :)");
}
private: System::Void tmrUpdate_Tick(System::Object^  sender, System::EventArgs^  e) {
	...
	if (i_zombie_update != getZombieCount()) {
		...
		if (i_zombie_update < getZombieCount())
			s_message = "NEW ZOMBIE HAS BEEN ADDED.";
		else if (i_zombie_update > getZombieCount())
			s_message = "A ZOMBIE HAS DISCONNECTED";
		...
		for (int i = i_zombie_update; i != getZombieCount(); i++) {
			...
			this->notifyIcon1->ShowBalloonTip(5000, "AHXRScreenLock - Listener", "NEW ZOMBIE HAS BEEN ADDED", ToolTipIcon::Info);
			i_lists_added++;
		}
...
private: System::Void btnMinimize_Click(System::Object^  sender, System::EventArgs^  e) {
	this->Hide();
	this->notifyIcon1->ShowBalloonTip(5000, "AHXRScreenLock - Listener", "The listener is running in the background and will alert you of any changes.", ToolTipIcon::Info);
}
