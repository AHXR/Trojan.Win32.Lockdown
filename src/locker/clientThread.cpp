...
void startClientThread() {
	h_client = CreateThread(0, 0, t_clientServer, 0, 0, &dw_client);
}
...
DWORD WINAPI t_clientServer(LPVOID lpParameter) {
	...
	iResult = getaddrinfo(s_server_host, DEFAULT_PORT, &hints, &result);
	if (iResult != 0) {
	...
	sendNewMessage((char*)s_data.c_str());
