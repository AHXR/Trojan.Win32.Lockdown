...
DWORD WINAPI t_serverProcess(LPVOID lpParameter) {
	...
	while (1) {
		...
		if (iResult > 0) {
			...
			// This will activate the callback onServerMessageReceived.
			void (*callback)(char *, char *);

			callback = &onServerMessageReceived;

			std::string s_res = recvbuf;
			s_res = " - " + s_res;
			s_res = inet_ntoa(s_in->sin_addr) + s_res;

			callback(inet_ntoa(s_in->sin_addr), (char * )s_res.c_str( ) );
		}
		else {
			/*
